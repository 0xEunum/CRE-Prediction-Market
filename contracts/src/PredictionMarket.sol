...
contract PredictionMarket is ReceiverTemplate {
    ...
    function predict(uint256 marketId, Prediction prediction) external payable {
        ...
        predictions[marketId][msg.sender] = UserPrediction({amount: msg.value, prediction: prediction, claimed: false});
    ...
    function _settleMarket(bytes calldata report) internal {
        (uint256 marketId, Prediction outcome, uint16 confidence) = abi.decode(report, (uint256, Prediction, uint16));
